...
4. `/skip_prefix`: 翻訳をスキップするためのプレフィックスを設定します。
   - 引数なしで実行すると、現在の設定を表示します。
   - プレフィックスを変更するには、`prefix`パラメータに新しい値を設定します。
...
特定のメッセージを翻訳したくない場合は、メッセージの先頭に設定されたプレフィックス（デフォルトは`;`）を付けます。例：

```
;このメッセージは翻訳されません
```

プレフィックスは `/skip_prefix` コマンドで変更できます。
